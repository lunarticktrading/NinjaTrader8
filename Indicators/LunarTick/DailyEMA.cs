...
namespace NinjaTrader.NinjaScript.Indicators.LunarTick
{
    ...
    public class DailyEMA : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = @"Plots the daily EMA on intraday charts.";
                Name = "DailyEMA";
                ...
                VersionInformation = $"{Version} - {Assembly.GetAssembly(typeof(DailyEMA)).GetName().Version}";
                Debug = false;

                AddPlot(Brushes.White, "DailyEMA");
            }
            ...
            {
                _dailyEMA = EMA(BarsArray[1], Period);
            }
        ...
        protected override void OnBarUpdate()
        {
            if (BarsInProgress == 0)
            {
                ...
                if (State == State.Realtime)
                {
                    ...
                    // Plot all DailyEMA values for the current day.
            ...
            else if (BarsInProgress == 1)
            {
                ...
                {
                    // Called once at close of historic bar, so just record daily EMA value.
                    if (CurrentBar > 0)
                    {
                        _dailyEMA.Update();
                        _historicDailyValues[CurrentBar] = new DailyValue(Time[1], Time[0], CurrentBar, _dailyEMA[0]);
                        DebugPrint($"Recorded historical DailyEMA [{CurrentBar}][{Time[0]}] = [{_dailyEMA[0]}]");
                    }
                }
                else if (State == State.Realtime)
                {
                    _dailyEMA.Update();
                    _lastDailyValue = _dailyEMA[0];
                }
            }
        }
        ...
        private void DebugPrint(string msg)
        {
            if (Debug)
            {
                if (Instrument != null && !string.IsNullOrWhiteSpace(Instrument.FullName))
                    Print($"DailyEMA[{Instrument.FullName}]: {msg}");
                else
                    Print($"DailyEMA: {msg}");
            }
        }
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private LunarTick.DailyEMA[] cacheDailyEMA;
		public LunarTick.DailyEMA DailyEMA(int period)
		{
			return DailyEMA(Input, period);
		}

		public LunarTick.DailyEMA DailyEMA(ISeries<double> input, int period)
		{
			if (cacheDailyEMA != null)
				for (int idx = 0; idx < cacheDailyEMA.Length; idx++)
					if (cacheDailyEMA[idx] != null && cacheDailyEMA[idx].Period == period && cacheDailyEMA[idx].EqualsInput(input))
						return cacheDailyEMA[idx];
			return CacheIndicator<LunarTick.DailyEMA>(new LunarTick.DailyEMA(){ Period = period }, input, ref cacheDailyEMA);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.LunarTick.DailyEMA DailyEMA(int period)
		{
			return indicator.DailyEMA(Input, period);
		}

		public Indicators.LunarTick.DailyEMA DailyEMA(ISeries<double> input , int period)
		{
			return indicator.DailyEMA(input, period);
		}
	}
